...
/**
 * Class \mod_mooduell\local\wunderbyte_table\output\table
 *
...
namespace mod_mooduell\local\wunderbyte_table\output;

use core_plugin_manager;
use mod_mooduell\local\wunderbyte_table\editfilter;
...
class table implements renderable, templatable {
    ...
    public function __construct(wunderbyte_table $table, string $encodedtable = '') {
        ...
        if (
            get_config('mod_mooduell', 'allowedittable')
            && has_capability('mod/mooduell:canedittable', $table->context)
        ) {
            $this->edittable = true;
        } else {
    ...
    private function showcountselect() {
        ...
        while ($counter <= 19) {
            ...
            array_push($options, [
                'label' => get_string('pagelabel', 'mod_mooduell', $pagenumber),
                'value' => $pagenumber,
    ...
    private function applyfilterselection(wunderbyte_table $table) {
        ...
        if (get_config('mod_mooduell', 'hideallfiltershavingbypasscache')) {
    ...
    private function add_readable_info_about_filter_and_search($filtercountarray, $table) {
        ...
        if (!empty($table->searchtext)) {
            ...
            $string .= get_string('searchcountmessage', 'mod_mooduell', $table->searchtext);
        }

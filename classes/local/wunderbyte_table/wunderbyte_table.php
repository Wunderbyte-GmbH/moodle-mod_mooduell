...
namespace mod_mooduell\local\wunderbyte_table;
use mod_mooduell\local\wunderbyte_table\local\performance\performance;
use mod_mooduell\local\wunderbyte_table\local\sortables\sortable_info;
...
use context_system;
use Exception;
use mod_mooduell\local\wunderbyte_table\output\lazytable;
...
use mod_mooduell\local\wunderbyte_table\event\template_switched;
use mod_mooduell\local\wunderbyte_table\filters\base;
use mod_mooduell\local\wunderbyte_table\local\sortables\base as basesort;
use mod_mooduell\local\wunderbyte_table\filters\types\standardfilter;
use mod_mooduell\local\wunderbyte_table\local\settings\tablesettings;
...
class wunderbyte_table extends table_sql {
     ...
     */
    public $cachecomponent = 'mod_mooduell';
     ...
     */
    public $tabletemplate = 'mod_mooduell/twtable_list'; // Default template.
     ...
     */
    public $requirecapability = 'mod/mooduell:canaccess';
    ...
    public function outhtml($pagesize, $useinitialsbar, $downloadhelpbutton = '') {
        ...
        /** @var \mod_mooduell\local\wunderbyte_table\output\renderer $output */
    ...
    public function printtable($pagesize, $useinitialsbar, $downloadhelpbutton = '') {
        ...
        if ($this->addcheckboxes && !$this->is_downloading()) {
            ...
            array_unshift($headers, get_string('tableheadercheckbox', 'mod_mooduell'));
            $this->columns = [];
        ...
        if ($this->sortablerows && !$this->is_downloading()) {
            ...
            array_unshift($headers, get_string('tableheadersortableitem', 'mod_mooduell'));
            $this->columns = [];
            $this->define_columns($columns);
            $this->headers = $headers;
        }
    ...
    private function query_db_cached_filtered(int $pagesize, bool $useinitialsbar, string $totalcountsql) {
        ...
        if (
            !get_config('mod_mooduell', 'turnoffcaching')
            && $this->cachecomponent
            && $this->rawcachename
            && !$this->bypasscache
        ) {
        ...
        if ($cachedrawdata !== false) {
            ...
            if (
                get_config('mod_mooduell', 'logfiltercaches')
                ...
                if (
                    ...
                    $DB->update_record('mod_mooduell', $record);
                }
            }
        } else {
            ...
            try {
                ...
                if ($CFG->debug > 0) {
                    ...
                    $this->errormessage = get_string('somethingwentwrong', 'mod_mooduell');
                }
            ...
            if (
                ...
                if (count($this->rawdata) > 0) {
                    ...
                    if (get_config('mod_mooduell', 'logfiltercaches')) {
                        ...
                        if (
                            ...
                            $DB->insert_record('mod_mooduell', $data);
                        }
                    }
                }
            }
    ...
    public function apply_filter(string $filter, string &$searchtext = '') {
        ...
        if ($filter !== "" && !$filterobject = json_decode($filter)) {
            throw new moodle_exception('invalidfilterjson', 'mod_mooduell');
        }
        ...
        if (get_config('mod_mooduell', 'allowsearchincolumns')) {
            if (!$searchtext == '') {
            ...
            if (!empty($categoryvalue)) {
                ...
                if (!empty($availablefilters->categories)) {
                    foreach ($availablefilters->categories as $category) {
                        ...
                        if (
                            $category->wbfilterclass === 'mod_mooduell\local\wunderbyte_table\filters\types\datepicker'
                            &&
    ...
    public function apply_searchtext(string $searchtext) {
        ...
        if (empty($searchtext)) {
            throw new moodle_exception('invalidsearchtext', 'mod_mooduell');
        }
        $this->searchtext = $searchtext;
    ...
    private function return_component_and_template() {
        ...
        if (empty($component) || empty($template)) {
            throw new moodle_exception('wrongtemplatespecified', 'mod_mooduell', '', $this->tabletemplate);
        }
    ...
    public function return_encoded_table($newcache = false) {
        ...
        if (empty($this->tablecachehash) || $newcache) {
            $cache = cache::make('mod_mooduell', 'encodedtables');
    ...
    public function col_wbcheckbox($values): string {
        ...
        return $OUTPUT->render_from_template('mod_mooduell/col_checkbox', $data);
        ;
    }
    ...
    public function col_wbsortableitem($values): string {
        ...
        return $OUTPUT->render_from_template('mod_mooduell/col_sortableitem', $data);
        ;
    }
     ...
     *
     * @param string $template The full template string, e.g. 'mod_mooduell/twtable_list'.
     ...
     *
     * @param string $template full template name, e.g. 'mod_mooduell/twtable_list'

...
namespace mod_mooduell\local\wunderbyte_table\privacy;
...
class provider implements core_userlist_provider, core_user_data_provider, metadata_provider, plugin_provider {
        ...
        foreach ($contextlist as $context) {
            if ($context->contextlevel == CONTEXT_SYSTEM) {
                ...
                writer::with_context($context)->export_data(
                    [get_string('privacy:metadata:local_wunderbyte_table', 'mod_mooduell')],
                    (object) $data
                );
            }
        }
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        foreach ($users as $user) {
            $contextlist = new approved_contextlist($user, 'mod_mooduell', [CONTEXT_SYSTEM]);

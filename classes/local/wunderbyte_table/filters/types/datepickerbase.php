...
abstract class datepickerbase extends base {
    ...
    public function add_options(
        ...
        if (!in_array($operator, ['=', '<', '>', '<=', '>='])) {
            throw new moodle_exception('novalidoperator', 'mod_mooduell');
        }

        $filter = [
            'checkboxlabel' => !empty($checkboxlabel) ? $checkboxlabel : get_string('apply_filter', 'mod_mooduell'),
        ];
        ...
        switch ($type) {
            ...
            case 'in between':
                ...
                break;
            default:
                throw new moodle_exception('unsupportedfiltertype', 'mod_mooduell');
        }
        ...
        foreach ($datepickerarray['datepicker'] as $labelkey => $object) {
            if (!isset($object['columntimestart'])) {
                ...
                $operationsarray = array_map(fn($y) => [
                    'operator' => $y,
                    'label' => get_string($y, 'mod_mooduell'),
    ...
    public static function non_kestringy_value_pair_properties($filtercolumn) {
        return [
            'localizedname',
            'wbfilterclass',
            'mod_mooduell\local\wunderbyte_table\filters\types\datepicker',
    ...
    public static function add_type_dropdown(&$mform, $selected) {
        ...
        $mform->addGroup(
            $categoryselect,
            'subdatepicker_group',
            get_string('datepickertype', 'mod_mooduell'),
            ' ',
            false
        );

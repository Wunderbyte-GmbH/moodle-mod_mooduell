...
class validation_manager extends filtersettings {
    ...
    private function checked_general_filter_settings($filterdata) {
        ...
        if (empty($filterdata['filter_columns'])) {
            $errros['filter_columns'] = get_string('columnemptyerror', 'mod_mooduell');
        }
        if (empty($filterdata['localizedname'])) {
            $errros['localizedname'] = get_string('localizednameemptyerror', 'mod_mooduell');
        }
        if (empty($filterdata['wbfilterclass'])) {
            $errros['wbfilterclass'] = get_string('wbfilterclassemptyerror', 'mod_mooduell');
        }
        return $errros;
    }
}

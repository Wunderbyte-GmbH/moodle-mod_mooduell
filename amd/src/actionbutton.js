...
import {showNotification} from 'mod_mooduell/notifications';
import {reloadAllTables} from 'mod_mooduell/reload';
import {
...
async function showNoCheckboxNotification() {
  const message = await getString('nocheckboxchecked', 'mod_mooduell');
...
export function transmitAction(id, methodname, datastring, idstring, encodedtable) {
  ...
  Ajax.call([{
    methodname: "mod_mooduell_execute_action",
    args: {
      'id': parseInt(id),
...
function getStringsFromDataset(button, datastring, noselection) {
  ...
  strings.push({
    ...
    component: button.dataset.component ?? 'mod_mooduell',
  });
  if (noselection) {
    strings.push({
